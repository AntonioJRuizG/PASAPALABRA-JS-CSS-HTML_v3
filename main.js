...
const inputsEndCondition = () => {
    letterDefinition.innerHTML = `Instructions: <br>Enter your user name. Then click START and enjoy! :).<br> Click on START again to restart the game or FINISH to change the user name. <br> There are infinite rounds to answer the skipped questions ;)`;
    timeField.value = '';
    scoreField.value = '';
...
const inputsShowingRankingCondition = () => {
    sendButton.disabled = true;
    skipTurn.disabled = true;
    userAnswer.disabled = true;
...
const restartQuestionsListIndex = () => {
    questionListPlace = 0;
}
...
const addTurn = () => {
    ...
    } while (questionsList[questionListPlace].status !== 0 && !checkIfAllQuestionsAnswerded());
}
...
const sortBiggestToSmallestObject = (obj) => {
...
const calculateRanking = () => {
    ...
    let time = maxTime - timeField.value;
    let userObj = {
        "user": user,
        "point": point,
        "time": time
    ...
    sortBiggestToSmallestObject(ranking);

    for (let i = 0; i < ranking.length; i++) {
        rankingList += `${i + 1}- ${ranking[i].user} got ${ranking[i].point} correct answers in ${ranking[i].time} seconds.<br>`;
    }
    finalMessage = `--------------- - - GAME COMPLETED! CONGRATS!!! - - ---------------- <br>RANKING:<br>` + rankingList;
    rankingList = '';
}
...
const askQuestion = (answer, i) => {
    if (answer.toLowerCase() === questionsList[i].answer.toLowerCase()) {
        ...
        questionsList[i].status = 1;
        gameLetters[i].classList.add('mistaken');
    }
}

const checkIfAllQuestionsAnswerded = () => {
    return questionsList.every(word => word.status !== 0);
}

const gameOver = () => {
    inputsShowingRankingCondition();
    stopTimer();
}
...
startGame.addEventListener("click", () => {
    ...
    inputsStartCondition();
    updateScore();
    startTimer();
    updateDisplay();
    restartQuestionsObject();
    restartAlphabetColors();
    restartQuestionsListIndex();
})
...
sendButton.addEventListener("click", () => {
    ...
    if (checkIfAllQuestionsAnswerded()) {
        calculateRanking();
        gameOver();
    }
});
